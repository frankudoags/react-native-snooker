import { Canvas, Group, Rect, Circle, Line, RadialGradient } from '@shopify/react-native-skia';
import {
  TABLE_WIDTH,
  TABLE_HEIGHT,
  TABLE_BORDER,
...
function TableMarkers() {
  ...
  return (
    <Group>
      ...
      <Circle cx={TABLE_WIDTH / 2} cy={TABLE_HEIGHT * 0.78} r={3} color="rgba(255,255,255,0.55)" />

      <Circle cx={TABLE_WIDTH / 2} cy={TABLE_HEIGHT * 0.28} r={3} color="rgba(255,255,255,0.55)" />
    </Group>
  );
}
  ...
  return (
    <>
      {POCKET_POSITIONS.map((pocket, index) => (
        <Group
          key={`pocket-${index}`}
          transform={[{ translateX: pocket.x }, { translateY: pocket.y }]}
        >
