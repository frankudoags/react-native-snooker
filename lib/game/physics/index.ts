...
class TableRailBuilder {
  public createRails(): Matter.Body[] {
    ...
    rails.push(
      this.createRail(leftX, (sideTopStart + sideTopEnd) / 2, thickness, sideTopEnd - sideTopStart)
    );
    rails.push(
      this.createRail(
        leftX,
        (sideBottomStart + sideBottomEnd) / 2,
        thickness,
        sideBottomEnd - sideBottomStart
      )
    );
    rails.push(
      this.createRail(rightX, (sideTopStart + sideTopEnd) / 2, thickness, sideTopEnd - sideTopStart)
    );
    rails.push(
      this.createRail(
        rightX,
  ...
  private findCueBallResetSpot(): { x: number; y: number } {
    ...
    for (const spot of candidates) {
      ...
      const colliding = Object.values(this.ballsState).some((ball) => {
        ...
        return (
          distanceSquared(candidate.x, candidate.y, ball.x, ball.y) <
          CUE_RESET_CLEARANCE * CUE_RESET_CLEARANCE
        );
      });
...
export function checkPockets(
  positions: Partial<Record<BallId, { x: number; y: number }>>
): Set<BallId> {
...
export function checkCueBallPocketed(
  positions: Partial<Record<BallId, { x: number; y: number }>>
): boolean {
